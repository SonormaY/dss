class NetworkSettings:
    ...
    INTERRUPT_ON_ERROR = (
        False  # Stop running simulation in case of an error during action processing
    )
    EXTERNAL_REQUEST_MODE = True  # Allow sending external requests to random node
    EXTERNAL_REQUEST_TOTAL_REQUESTS_NUMBER = (
        40  # Limit of created external requests. If none the number is unlimited
    )
    ...
    WEIGHTS = [
        ...
        0.25,
        0.25,
    ]  # List of weights (chances) for the NUMBER_OF_REQUESTS value to be chosen.
